...
for angle in range(60, 61):
    ...
    name = "temporal_sphere_reuse_sms_spatial_unique_{:02d}_angle{}".format(spatial_reuse_size, angle)
    cmd = "mitsuba "
    cmd += "sphere_sms.xml "
    ...
    cmd += "-Dcaustics_temporal_path_reuse=results_sphere_angle_{} ".format(angle-1)
    cmd += "-Dcaustics_current_path_reuse=results_sphere_angle_{} ".format(angle)
    
    cmd += "-Dcaustics_temporal_reuse=true "
