...
public:
    ...
    SingleScatterSMSPathIntegrator(const Properties &props) : Base(props) {
        ...
        m_sms_config.reuse                  = props.bool_("reuse", false);  //add for spatiotemporal_reuse
        ...
        if constexpr (is_array_v<Float>) {
            ...
            for (int depth = 1;; ++depth) {
                ...
                if (si.shape->is_caustic_receiver() && !on_caustic_caster &&
                    (m_max_depth < 0 || depth + 1 < m_max_depth)) {
                    SpecularManifoldSingleScatter mf(scene, m_sms_config);
                    result += throughput * mf.specular_manifold_sampling_reuse(si, sampler, depth, pixel_records_w, pixel_records_r);
                }
                ...
                BSDFContext ctx;
                //xxf add debug
                //ctx.type_mask = (uint32_t)BSDFFlags::DeltaTransmission; 
                //ctx.type_mask = (uint32_t)BSDFFlags::DeltaReflection; 
                ctx.sampler = sampler;
                BSDFPtr bsdf = si.bsdf(ray);
