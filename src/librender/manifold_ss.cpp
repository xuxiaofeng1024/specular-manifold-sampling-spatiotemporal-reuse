...
//Information used to count the average number of Newton iterations
MTS_VARIANT std::atomic<size_t> SpecularManifoldSingleScatter<Float, Spectrum>::stats_first_newton_solver_calls(0);
MTS_VARIANT std::atomic<size_t> SpecularManifoldSingleScatter<Float, Spectrum>::stats_first_newton_solver_iterations(0);
MTS_VARIANT std::atomic<size_t> SpecularManifoldSingleScatter<Float, Spectrum>::stats_second_newton_solver_calls(0);
MTS_VARIANT std::atomic<size_t> SpecularManifoldSingleScatter<Float, Spectrum>::stats_second_newton_solver_iterations(0);

MTS_VARIANT std::atomic<size_t> SpecularManifoldSingleScatter<Float, Spectrum>::stats_total_solutions(0);
...
MTS_VARIANT void 
SpecularManifoldSingleScatter<Float, Spectrum>::get_reuse_info(const int depth, const PixelCacheInfo &pixel_records_r, 
                                                                            std::vector<Point3f>& points_reuse, int& trials) {
    switch(depth) {
        case 1:
            trials = pixel_records_r.m[0];
            points_reuse.resize(trials);
            points_reuse.assign(pixel_records_r.points0.begin(), pixel_records_r.points0.end());
            break;
        case 2:
            trials = pixel_records_r.m[1];
            points_reuse.resize(trials);
            points_reuse.assign(pixel_records_r.points1.begin(), pixel_records_r.points1.end());
            break;
        case 3:
            trials = pixel_records_r.m[2];
            points_reuse.resize(trials);
            points_reuse.assign(pixel_records_r.points2.begin(), pixel_records_r.points2.end());
            break;
        case 4:
            trials = pixel_records_r.m[3];
            points_reuse.resize(trials);
            points_reuse.assign(pixel_records_r.points3.begin(), pixel_records_r.points3.end());
            break;
        case 5:
            trials = pixel_records_r.m[4];
            points_reuse.resize(trials);
            points_reuse.assign(pixel_records_r.points4.begin(), pixel_records_r.points4.end());
            break;
        default:
            trials = m_config.max_trials;
            std::cout << "reuse default " << trials << std::endl;
            break;
    }
}

MTS_VARIANT Spectrum
SpecularManifoldSingleScatter<Float, Spectrum>::specular_manifold_sampling_reuse(const SurfaceInteraction3f &si,
                                                                                 ref<Sampler> sampler, 
                                                                                 int depth,
    ...
    for (size_t shape_idx = 0; shape_idx < shapes.size(); ++shape_idx) {
        ...
        if (!m_config.biased) {
            // Unbiased SMS
            Mask success;
            SurfaceInteraction3f si_final;
            SurfaceInteraction3f unused;

            // Sample a single path
                auto [success_0, si_final_0, unused_0] = sample_path(specular_shape, si, ei, sampler, n_offset);
                success = success_0;
                si_final = si_final_0;
                unused = unused_0;
            ...
            if(!m_config.reuse) {
                ...
                for (int m = 0; m < m_config.max_trials; ++m) {
                    ...
                    // We already know there is no obstacle between si & si_final, but we
                    // need to explicitly check visibility between the si_final and the
                    // light source.
                    Ray3f ray_vis;
                    if (ei.is_directional()) {
                ...
                stats_total_solutions += solutions.size(); 

                switch(depth){
                    case 1:
                        pixel_records_w.m[0] = points.size();
                        pixel_records_w.si_point[0] = si.p;
                        pixel_records_w.points0.resize(points.size());
                        pixel_records_w.points0.assign(points.begin(), points.end());
                        break;
                    case 2:
                        pixel_records_w.m[1] = points.size();
                        pixel_records_w.si_point[1] = si.p;
                        pixel_records_w.points1.resize(points.size());
                        pixel_records_w.points1.assign(points.begin(), points.end());
                        break;
                    case 3:
                        pixel_records_w.m[2] = points.size();
                        pixel_records_w.si_point[2] = si.p;
                        pixel_records_w.points2.resize(points.size());
                        pixel_records_w.points2.assign(points.begin(), points.end());
                        break;
                    case 4:
                        pixel_records_w.m[3] = points.size();
                        pixel_records_w.si_point[3] = si.p;
                        pixel_records_w.points3.resize(points.size());
                        pixel_records_w.points3.assign(points.begin(), points.end());
                        break;
                    case 5:
                        pixel_records_w.m[4] = points.size();
                        pixel_records_w.si_point[4] = si.p;
                        pixel_records_w.points4.resize(points.size());
                        pixel_records_w.points4.assign(points.begin(), points.end());
                        break;
                    default:
                        std::cout << "depth = default"  << std::endl;
                        std::cout << "write case default" << std::endl;
                        break;
                }
            }
            // reuse 1spp biased SMS pilot 
            else if (m_config.reuse){ 
                std::vector<Vector3f> solutions;
                std::vector<Point3f> points;

                int trials; 
                std::vector<Point3f> points_reuse;

                get_reuse_info(depth, pixel_records_r, points_reuse, trials);
                ...
                for (int m = 0; m < trials; ++m) {
                    ...
                    // Reuse a recorded path
                    if(m < points_reuse.size()){
                        auto [success_0, si_final_0, unused_0] = sample_path_reuse(specular_shape, si, ei, points_reuse[m], sampler, n_offset);
                        success = success_0;
                        si_final = si_final_0;
                        unused = unused_0;
                    }
...
SpecularManifoldSingleScatter<Float, Spectrum>::sample_path(const ShapePtr shape,
      ...
      ScopedPhase scope_phase(ProfilerPhase::SMSSamplePath);
    ...
    if (m_config.twostage) {
        ...
        stats_first_newton_solver_calls++;

        auto [success_smooth, si_smooth] = newton_solver_first(si, vtx_init, ei, lean_normal_local, 1.f);
    ...
    stats_second_newton_solver_calls++;
    auto [success, si_final] = newton_solver_second(si, vtx_init, ei, n_offset, 0.f);
...
MTS_VARIANT std::tuple<typename SpecularManifoldSingleScatter<Float, Spectrum>::Mask,
                       typename SpecularManifoldSingleScatter<Float, Spectrum>::SurfaceInteraction3f,
                       typename SpecularManifoldSingleScatter<Float, Spectrum>::SurfaceInteraction3f>
SpecularManifoldSingleScatter<Float, Spectrum>::sample_path_reuse(const ShapePtr shape,
                                                            const SurfaceInteraction3f &si,
                                                            const EmitterInteraction &ei,
                                                            const Point3f &position_reuse,
                                                            ref<Sampler> sampler,
                                                            const Vector3f &n_offset,
                                                            const Point3f &p_start) const {
      ScopedPhase scope_phase(ProfilerPhase::SMSSamplePathReuse);

    //reuse 1spp biased SMS pilot run information
    PositionSample3f ps;
    ps.p = position_reuse;
    Vector3f d_tmp = normalize(ps.p - si.p);
    ...
    ManifoldVertex vtx_init = ManifoldVertex(si_init, 0.f);
...
MTS_VARIANT std::pair<typename SpecularManifoldSingleScatter<Float, Spectrum>::Mask,
            typename SpecularManifoldSingleScatter<Float, Spectrum>::SurfaceInteraction3f>
            SpecularManifoldSingleScatter<Float, Spectrum>::newton_solver_first(const SurfaceInteraction3f &si,
                                                              const ManifoldVertex &vtx_init,
                                                              const EmitterInteraction &ei,
                                                              const Vector3f &n_offset,
                                                              Float smoothing) const {
    ScopedPhase scope_phase(ProfilerPhase::SMSNewtonSolverFirst);
    ...
    stats_first_newton_solver_iterations += iterations;
...
MTS_VARIANT std::pair<typename SpecularManifoldSingleScatter<Float, Spectrum>::Mask,
                      typename SpecularManifoldSingleScatter<Float, Spectrum>::SurfaceInteraction3f>
SpecularManifoldSingleScatter<Float, Spectrum>::newton_solver_second(const SurfaceInteraction3f &si,
                                                              const ManifoldVertex &vtx_init,
                                                              const EmitterInteraction &ei,
                                                              const Vector3f &n_offset,
                                                              Float smoothing) const {
    ScopedPhase scope_phase(ProfilerPhase::SMSNewtonSolverSecond);
    ManifoldVertex vtx = vtx_init;
    ...
    SurfaceInteraction3f si_current;
    Matrix2f dV1_dC(0.f), dC_dV2(0.f);
    while (iterations < m_config.max_iterations) {
        bool step_success;
        Vector2f C;
        Vector2f dX;
    ...
    stats_second_newton_solver_iterations += iterations;
...
//Not used
// rewrite method compute_step_halfvector to obtain the reuse information 
MTS_VARIANT std::tuple<typename SpecularManifoldSingleScatter<Float, Spectrum>::Mask,
                       typename SpecularManifoldSingleScatter<Float, Spectrum>::Vector2f,
                       typename SpecularManifoldSingleScatter<Float, Spectrum>::Vector2f,
                       typename SpecularManifoldSingleScatter<Float, Spectrum>::Matrix2f,
                       typename SpecularManifoldSingleScatter<Float, Spectrum>::Matrix2f>
SpecularManifoldSingleScatter<Float, Spectrum>::compute_step_halfvector_reuse(const Point3f &v0p,
                                                                              const ManifoldVertex &v1,
                                                                              const EmitterInteraction &v2,
                                                                              const Vector3f &n_offset,
                                                                              Float time,
    ...
    Float ilo = norm(wo);
    if (ilo < 1e-3f) {
        return std::make_tuple(false, Vector2f(math::Infinity<Float>), Vector2f(0.f), Matrix2f(0.f), Matrix2f(0.f));
    }
    ilo = rcp(ilo);
    wo *= ilo;
    ...
    // Invert matrix dC_dX1
    Float determinant_dC_dX1 = det(dC_dX1);
    if (abs(determinant_dC_dX1) < 1e-6f) {
        return std::make_tuple(false, Vector2f(math::Infinity<Float>), Vector2f(0.f), Matrix2f(0.f), Matrix2f(0.f));
    }
    Matrix2f dX1_dC = inverse(dC_dX1);
    ...
    //add Prepare emitter interaction for Derivative and offset computation
    auto[success_e, vy] = SpecularManifold::emitter_interaction_to_vertex(m_scene, v2, v1.p, time, wavelengths);
    
   // Local shading tangent frame of vy
    Float vy_dot_dpdu_n = dot(vy.dp_du, vy.n),
          vy_dot_dpdv_n = dot(vy.dp_dv, vy.n);
    Vector3f vy_s = vy.dp_du - vy_dot_dpdu_n * vy.n,
             vy_t = vy.dp_dv - vy_dot_dpdv_n * vy.n;

    //add Derivative of specular constraint w.r.t. v2
    ...
    return std::make_tuple(true, dH, dX, dX1_dC, dC_dX2);
}

//Not used
//method compute_tangents() to obtain the reuse information 
MTS_VARIANT std::tuple<typename SpecularManifoldSingleScatter<Float, Spectrum>::Matrix2f,
                       typename SpecularManifoldSingleScatter<Float, Spectrum>::Vector3f,
                       typename SpecularManifoldSingleScatter<Float, Spectrum>::Vector3f>
SpecularManifoldSingleScatter<Float, Spectrum>::compute_tangents(const SurfaceInteraction3f &si,
                                                                 const EmitterInteraction &v2,
                                                                 const SurfaceInteraction3f &si_final) const {

    // Specular point to ManifoldVertex
    ManifoldVertex v1(si_final);
    v1.make_orthonormal(); 

    // Emitter to ManifoldVertex
    auto[success_e, vy] = SpecularManifold::emitter_interaction_to_vertex(m_scene, v2, v1.p, si.time, si.wavelengths);
    if (!success_e) {
        return {Matrix2f(0.f), Vector3f(0.f), Vector3f(0.f)};
    }
    ...
    Matrix2f X1_Tp = -dX1_dC * dC_dX2;
    return {X1_Tp, vy.dp_du, vy.dp_dv};
}
...
MTS_VARIANT std::tuple<typename SpecularManifoldSingleScatter<Float, Spectrum>::Mask,
                       ...
                       typename SpecularManifoldSingleScatter<Float, Spectrum>::Matrix2f,
                       typename SpecularManifoldSingleScatter<Float, Spectrum>::Matrix2f>
SpecularManifoldSingleScatter<Float, Spectrum>::compute_step_anglediff_reuse(const Point3f &v0p,
                                                                             const ManifoldVertex &v1,
                                                                             const EmitterInteraction &v2,
                                                                             const Vector3f &n_offset,
                                                                             Float time, 
    ...
    // Compute step
    Vector2f dX = dX_dC * C;
    return std::make_tuple(success, C, dX, Matrix2f(0.f), Matrix2f(0.f));
}
...
MTS_VARIANT void SpecularManifoldSingleScatter<Float, Spectrum>::print_statistics() {
    ...
    Float stats_newton_solver_first_avg_iterations = Float(stats_first_newton_solver_iterations) / stats_first_newton_solver_calls;
    std::cout << std::setw(25) << std::left << "avg. First newton iterations: "
              << std::setw(10) << std::right << stats_newton_solver_first_avg_iterations << std::endl;
    Float stats_newton_solver_second_avg_iterations = Float(stats_second_newton_solver_iterations) / stats_second_newton_solver_calls;
    std::cout << std::setw(25) << std::left << "avg. Second newton iterations: "
              << std::setw(10) << std::right << stats_newton_solver_second_avg_iterations << std::endl;
    
    std::cout << std::setw(25) << std::left << "total solutions: "
              << std::setw(10) << std::right << stats_total_solutions << std::endl;
