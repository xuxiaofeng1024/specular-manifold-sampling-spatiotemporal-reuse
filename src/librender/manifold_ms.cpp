...
MTS_VARIANT std::atomic<size_t> SpecularManifoldMultiScatter<Float, Spectrum>::stats_newton_solver_calls(0);
MTS_VARIANT std::atomic<size_t> SpecularManifoldMultiScatter<Float, Spectrum>::stats_newton_solver_iterations(0);
MTS_VARIANT std::atomic<size_t> SpecularManifoldMultiScatter<Float, Spectrum>::stats_total_solutions(0);
...
MTS_VARIANT void 
SpecularManifoldMultiScatter<Float, Spectrum>::get_reuse_info(const int depth, const PixelCacheInfo &pixel_records_r, 
...
MTS_VARIANT Spectrum
SpecularManifoldMultiScatter<Float, Spectrum>::specular_manifold_sampling_reuse(const SurfaceInteraction3f &si,
                                                                          ref<Sampler> sampler,
                                                                          int depth,
        ...
        if (!m_config.biased) {
            ...
            if (!m_config.reuse){
                ...
                for (int m = 0; m < m_config.max_trials; ++m) {
                    ...
                    // Record this new solution
                    solutions.push_back(direction);
                    points.push_back(m_current_path[0].p);
                ...
                for (int m = 0; m < trials; ++m) {
                    ...
                    if(m < points_reuse.size()){
                        ...
                        success = sample_path_reuse(specular_shape, si, ei, points_reuse[m], sampler, m_seed_path.size() == 0);
                    }
                    else{
                        // Sample path, but only sample offset normals when we couldn't establish a seed path yet
                        success = sample_path(specular_shape, si, ei, sampler, m_seed_path.size() == 0);
...
MTS_VARIANT typename SpecularManifoldMultiScatter<Float, Spectrum>::Mask
SpecularManifoldMultiScatter<Float, Spectrum>::sample_path_reuse(const ShapePtr shape,
                                                           const SurfaceInteraction3f &si,
                                                           const EmitterInteraction &ei,
    ...
    Mask success_seed = sample_seed_path_reuse(shape, si, ei, position_reuse, sampler, first_path, p_start);
    if (!success_seed) {
        return false;
    }
...
MTS_VARIANT typename SpecularManifoldMultiScatter<Float, Spectrum>::Mask
SpecularManifoldMultiScatter<Float, Spectrum>::sample_seed_path_reuse(const ShapePtr shape,
                                                                const SurfaceInteraction3f &si_,
                                                                const EmitterInteraction &ei,
                                                                const Point3f &position_reuse,
                                                                ref<Sampler> sampler,
    ...
    // reuse biased SMS pilot run information
    Point3f x0 = si.p;
    Point3f x1 = position_reuse;
    if (any(neq(p_start, 0.f))) {
        x1 = p_start;
    }
    Vector3f wo = normalize(x1 - x0);
...
SpecularManifoldMultiScatter<Float, Spectrum>::newton_solver(const SurfaceInteraction3f &si,
                                                             const EmitterInteraction &ei) {
    ScopedPhase scope_phase(ProfilerPhase::SMSNewtonSolver);
    // Newton iterations..
    bool success = false;
    size_t iterations = 0;
    Float beta = 1.f;
    ...
    stats_newton_solver_iterations += iterations;
    if (!success) {
        return false;
    }
...
MTS_VARIANT void SpecularManifoldMultiScatter<Float, Spectrum>::print_statistics() {
    ...
    std::cout << std::endl;
    Float stats_newton_solver_avg_iterations = Float(stats_newton_solver_iterations) / stats_newton_solver_calls;
    std::cout << std::setw(25) << std::left << "avg. newton iterations: "
              << std::setw(10) << std::right << stats_newton_solver_avg_iterations << std::endl;
