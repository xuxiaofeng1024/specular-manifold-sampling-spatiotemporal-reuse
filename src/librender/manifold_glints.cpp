...
MTS_VARIANT Spectrum
SpecularManifoldGlints<Float, Spectrum>::specular_manifold_sampling_reuse(const Point3f &sensor_position,
                                                                           const SurfaceInteraction3f &si,
                                                                           ref<Sampler> sampler,
                                                                           PixelCacheGlintInfo &pixel_records_w,
                                                                           PixelCacheGlintInfo &pixel_records_r){
    ScopedPhase scope_phase(ProfilerPhase::SMSGlints);
    ...
    if (!m_scene->ray_test(ray_emitter)) {
        ...
        if (!m_config.biased) {
            ...
            pixel_records_w.m = solutions.size();
            pixel_records_w.uv_solutions.resize(solutions.size());
            pixel_records_w.uv_solutions.assign(solutions.begin(), solutions.end());

            sms_success = solutions.size() > 0;
        }
            else if (m_config.reuse){
            std::vector<Point2f> solutions;
            int trials = pixel_records_r.m;;
            std::vector<Point2f> uv_points_reuse;
            uv_points_reuse.resize(trials);
            uv_points_reuse.assign(pixel_records_r.uv_solutions.begin(), pixel_records_r.uv_solutions.end());

            for (int m = 0; m < trials; ++m) {

                //auto [success, uv_final, unused] = sample_glint(sensor_position, ei, si, sampler, n_offset);
                auto [success, uv_final, unused] = sample_glint_reuse(sensor_position, ei, si, uv_points_reuse[m], sampler, n_offset);
                if (!success) {
                    stats_solver_failed++;
                    continue;
                }
            ...
            pixel_records_r.m = solutions.size();
            pixel_records_r.uv_solutions.resize(solutions.size());
            pixel_records_r.uv_solutions.assign(solutions.begin(), solutions.end());
...
MTS_VARIANT std::tuple<typename SpecularManifoldGlints<Float, Spectrum>::Mask,
                       typename SpecularManifoldGlints<Float, Spectrum>::Point2f,
                       typename SpecularManifoldGlints<Float, Spectrum>::Point2f>
SpecularManifoldGlints<Float, Spectrum>::sample_glint_reuse(const Point3f &sensor_position,
                                                      const EmitterInteraction &ei,
                                                      const SurfaceInteraction3f &si,
                                                      const Point2f &uv_points_reuse,
                                                      ref<Sampler> sampler,
                                                      const Vector3f &n_offset,
                                                      const Point2f xi_start) const {
    // Check sides
    ...
    Point2f uv_init = si.uv + duv;
    */
    Point2f uv_init = uv_points_reuse;
