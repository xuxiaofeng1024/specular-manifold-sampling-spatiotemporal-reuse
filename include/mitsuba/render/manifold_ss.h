...
public:
    ...
    using MutationRecord     = MutationRecord<Float, Spectrum>;
    using PixelCacheInfo     = PixelCacheInfo<Float, Spectrum>;
    ...
    Spectrum specular_manifold_sampling_reuse(const SurfaceInteraction3f &si,
                                              ref<Sampler> sampler,
                                              int depth,
    ...
    std::tuple<Mask, SurfaceInteraction3f, SurfaceInteraction3f>
    sample_path_reuse(const ShapePtr shape,
                const SurfaceInteraction3f &si,
                const EmitterInteraction &ei,
    ...
    /// Newton solver to find admissable path segment
    std::pair<Mask, SurfaceInteraction3f>
    newton_solver_first(const SurfaceInteraction3f &si,
                  const ManifoldVertex &vtx_init,
                  const EmitterInteraction &ei,
                  const Vector3f &n_offset = Vector3f(0.f, 0.f, 1.f),
                  Float smoothing = 0.f) const;
    ...
    /// Newton solver to find admissable path segment
    std::pair<Mask, SurfaceInteraction3f>
    //std::tuple<Mask, SurfaceInteraction3f, Matrix2f, Matrix2f>
    newton_solver_second(const SurfaceInteraction3f &si,
                  const ManifoldVertex &vtx_init,
                  const EmitterInteraction &ei,
                  const Vector3f &n_offset = Vector3f(0.f, 0.f, 1.f),
                  Float smoothing = 0.f) const;
    ...
    std::tuple<Mask, Vector2f, Vector2f, Matrix2f, Matrix2f>
        compute_step_halfvector_reuse(const Point3f &v0p,
                                      const ManifoldVertex &v1,
                                      const EmitterInteraction &v2,
                                      const Vector3f &n_offset = Vector3f(0.f, 0.f, 1.f),
                                      Float time = 0.f,
                                      const Wavelength &wavelengths = 0.f) const; 
    ...
    // rewrite method compute_angle_diff to obtain the reuse information 
    std::tuple<Mask, Vector2f, Vector2f, Matrix2f, Matrix2f>
        compute_step_anglediff_reuse(const Point3f &v0p,
                                const ManifoldVertex &v1,
                                const EmitterInteraction &v2,
                                const Vector3f &n_offset = Vector3f(0.f, 0.f, 1.f),
                                Float time = 0.f,
    ...
    std::tuple<Matrix2f, Vector3f, Vector3f>
        compute_tangents(const SurfaceInteraction3f &si,
                           const EmitterInteraction &v2,
                           const SurfaceInteraction3f &si_final) const; 
    ...
    void get_reuse_info(const int depth, const PixelCacheInfo &pixel_records_r, std::vector<Point3f>& points_reuse, int& trials);
...
protected:
    ...
    //add 
    static std::atomic<size_t> stats_first_newton_solver_calls;
    static std::atomic<size_t> stats_first_newton_solver_iterations;
    static std::atomic<size_t> stats_second_newton_solver_calls;
    static std::atomic<size_t> stats_second_newton_solver_iterations;

    static std::atomic<size_t> stats_total_solutions;
protected:
